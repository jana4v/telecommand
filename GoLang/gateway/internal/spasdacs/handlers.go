...
// NewSpasdacsHandler wires a SpasdacsHandler with its dependencies.
func NewSpasdacsHandler(store repository.SpasdacsStore, logger *slog.Logger) *SpasdacsHandler {
	return &SpasdacsHandler{store: store, logger: logger}
}
...
func (h *SpasdacsHandler) GetDiagrams(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		common.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "internal server error"})
		return
	}
	common.WriteJSON(w, http.StatusOK, diagrams)
}

func (h *SpasdacsHandler) GetDiagram(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	if id == "" {
		common.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "id is required"})
		return
	}

	diagram, err := h.store.Get(r.Context(), id)
	if errors.Is(err, repository.ErrNotFound) {
		common.WriteJSON(w, http.StatusNotFound, map[string]string{"error": "Diagram not found"})
		return
	}
	if err != nil {
	...
	// ?slim=view: viewer-only fetch — drop the raw editor modelData (often
	// hundreds of KB) when the view-optimised copy exists. The client falls
	// back to modelData when viewModelData is absent, so only strip when the
	// replacement is actually present.
	if r.URL.Query().Get("slim") == "view" && diagram.ViewModelData != "" {
		diagram.ModelData = nil
	}

	common.WriteJSON(w, http.StatusOK, diagram)
}
...
func (h *SpasdacsHandler) PostDiagram(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&diagram); err != nil {
		...
		common.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid request body"})
		return
	}
	if diagram.ID == "" || diagram.Name == "" || diagram.ModelData == nil {
		common.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "Missing required fields: id, name, modelData"})
		return
	}
	...
	if err := h.store.Save(r.Context(), diagram); err != nil {
		...
		common.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to save diagram"})
		return
	}
...
func (h *SpasdacsHandler) PatchDiagram(w http.ResponseWriter, r *http.Request) {
	...
	if !hasAutoViewInclude && !hasAutoViewDuration {
		common.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "no patchable fields provided"})
		return
	}
	...
	common.WriteJSON(w, http.StatusOK, map[string]string{"success": "true"})
}
...
func (h *SpasdacsHandler) DeleteDiagram(w http.ResponseWriter, r *http.Request) {
	...
	common.WriteJSON(w, http.StatusOK, map[string]string{"success": "true", "message": "Diagram deleted"})
}
