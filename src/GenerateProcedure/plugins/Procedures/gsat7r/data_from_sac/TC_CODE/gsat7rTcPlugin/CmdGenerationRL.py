...
class DigitalProcessorRL:
    ...
    def generate_ch_1st_nco_band_select_words(self,cfg_number:int) -> list:
        ...
        """
        NCO1 = self.DP_CFG.filter(pl.col("CONFIG") == cfg_number).select(pl.col(["SNO","CONFIG","RL_CHANNELIZER_IP_PORTS","RL_CHANNELIZER_IP_FREQUENCIES"]))
    ...
    def generate_ch_2nd_nco_band_select_words(self, cfg_number: int) -> List[List[str]]:
        ...
        nco2 = self.DP_CFG.filter(pl.col("CONFIG") == cfg_number).select(pl.col(["RL_CHANNELIZER_OP_PORTS", "RL_CHANNELIZER_OP_NCO_COUNT"])).to_dict(as_series=False)
    ...
    def generate_com_and_power_channel_select_words(self, cfg_number: int) -> List[List[str]]:
        ...
        """
        # Get the NCO2 frequencies for the given configuration
        com_channels = (self.DP_CFG.filter(pl.col("CONFIG") == cfg_number)["RL_COM_CHANNEL_SELECT"].drop_nulls().to_list())
        pow_channels = (self.DP_CFG.filter(pl.col("CONFIG") == cfg_number)["RL_POWER_CHANNEL_SELECT"].drop_nulls().to_list())
        # Create the command list
    ...
    def generate_sub_channel_select_words(self, cfg_number: int) -> List[str]:
        ...
        sub_channel_code = (self.DP_CFG.filter(pl.col("CONFIG") == cfg_number)["RL_SUB_CHANNEL"].drop_nulls().to_list())[0].zfill(16)
    ...
    def generate_gain_word_for_rl_channels(self,cfg_number:int):
        ...
        com_channel_gains = (self.DP_CFG.filter(pl.col("CONFIG") == cfg_number)["RL_GAIN_CONTROL_DB"].drop_nulls().to_list())
        code_word = ""
    ...
    def generate_synthesizer_nco_word(self,cfg_number:int):
        ...
        """
        nco_counts = (self.DP_CFG.filter(pl.col("CONFIG") == cfg_number)["RL_SYNTHESIZER_NCO_COUNT"].drop_nulls().to_list())
        code_word = ""
