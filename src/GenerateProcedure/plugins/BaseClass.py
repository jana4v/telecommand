...
class ProcedureBase(ReadDatabase):
    ...
    async def get_procedure(self, row: TcDetailsTblRow, tc_request: TcRequest, is_start: bool,r) -> TestProcedure:
        ...
        #g7r = "".join(tc_request.commands) # GSAT7R SPECIFIC CODE
    ...
    async def generate_expected_command_file_format2(self, expected_tms: List[str], command="EXPECTED TM",is_start = True) -> str:
        # return ""
        try:
            #is_start = True
            expected = ""
            ...
            for tms in expected_tms:
                if tms is None:
                    continue
                for tm_condition in tms.split(","):
                    is_greater_than = False
                    # Parse the condition to extract TM, operator, and value
                    tm_condition = tm_condition.strip()
                    ...
                    for op in ['>=', '<=', '==']:
                        pos = tm_condition.find(op)
                        if pos != -1:
                            operator_found = op
                            operator_pos = pos
                            break
                    
                    # If not found, check for single-character operators
                    ...
                    # Split the condition
                    tm = tm_condition[:operator_pos].strip()
                    tm_state = tm_condition[operator_pos + len(operator_found):].strip()
                    
                    # Handle the operator mapping (convert == to = for output)
                    display_operator = '=' if operator_found == '==' else operator_found
                    
                    await self.inject_tm(tm, tm_state, self.tc_request)
                    if self.inject_tm_df.filter(pl.col("TM_MNEMONIC") == tm).is_empty():
                        if is_start:
                            expected += f"""{self.get_line_number()} {command}{self.get_spaces(command)}{self.get_orignal_tm_mnemonic(tm)} {display_operator} {tm_state};\n"""
                            is_start = False
                        else:
                            expected += f"{self.get_spaces()}{self.get_orignal_tm_mnemonic(tm)} {display_operator} {tm_state};\n"
            return expected[:-2] + "\n" if len(expected) > 3 else expected
        except Exception as e:
            return ""
    ...
    async def generate_send_file_format(self,commands:List[str],command_type:str="send")-> str:
        ...
        for command in commands:
            if command is not None and command != "" or command_type == "SINGLE_STEP":
                if is_start:
