...
class Send(ProcedureBase):
    ...
    async def _generate_ccl(self,ccl_cmd: str, tc_row: TcDetailsTblRow,is_start=True) -> str:
        ...
        if is_start:
            ...
            procedure += f"""{self.get_spaces()}{self.get_orignal_tc_mnemonic(tc)};\n"""
        data=tc.split(" ")
        if len(data) > 1:
            data = data[1]
        else:
            data = data[0]
        await self.inject_tc(tc,data)
        return procedure
    ...
    async def get_procedure(self, row: TcDetailsTblRow, tc_request: TcRequest, is_start: bool,r) -> TestProcedure:
        ...
        procedure.send_part = await self._generate_ccl("send", row, is_start)
...
if __name__ == "__main__":
    ...
    plugins_factory.register_component("send", Send)