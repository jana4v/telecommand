...
class Macro(ProcedureBase):
    ...
    async def _generate_ccl(self,ccl_cmd: str, tc_row: TcDetailsTblRow,is_start=True) -> str:
        ...
        original_tc = self.get_orignal_tc_mnemonic(tc)
        macro_expansion = self.get_macro_expansion(original_tc)
        if macro_expansion:
            original_tc = macro_expansion
            ccl_cmd = "send"
        if is_start:
                procedure += f"""{self.get_line_number()} {ccl_cmd}{self.get_spaces(ccl_cmd)}{original_tc};\n"""
        else:
            procedure += f"""{self.get_spaces()}{original_tc};\n"""
        if macro_expansion:
            commands = macro_expansion.split(",")
            for command in commands:
                data_command = command.split(" ")
                if len(data_command) == 2:
                    await self.inject_tc(data_command[0],data_command[1])
        return procedure
    ...
    async def get_procedure(self, row: TcDetailsTblRow, tc_request: TcRequest, is_start: bool,r) -> TestProcedure:
        ...
        procedure.send_part = await self._generate_ccl("macro", row, is_start)
...
if __name__ == "__main__":
    ...
    plugins_factory.register_component("macro", Macro)