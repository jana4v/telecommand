...
class ReadDatabase(Utility):
    ...
    def __init__(self):
        ...
        self.inhibited_cfgs = self.clean_df(pl.read_database("SELECT CFG_NO FROM INHIBITED_CONFIGS",cnxn,infer_schema_length=None))["CFG_NO"].to_list()
        self.tc_details_df = self.clean_df(pl.read_database("SELECT * FROM TeleCommandDetails",cnxn,infer_schema_length=None))
        self.tc_details_df = self.tc_details_df.unique(subset=["TC"], keep="first")
        self.inhibited_commands = self.tc_details_df.filter(pl.col("INHIBITED") == 1)["TC"].to_list()
        self.tc_details_df = self.tc_details_df.filter(pl.col("INHIBITED") == 0)
        ...
        cmd_inhibited_cfgs_df = cfg_based_tc_df.filter(pl.col("TC").is_in(self.inhibited_commands))
        cmd_inhibited_cfgs = cmd_inhibited_cfgs_df["CFG_NO"].unique().to_list()
        self.inhibited_cfgs = list(set(self.inhibited_cfgs) | set(cmd_inhibited_cfgs))
        self.create_inhibit_details_excel(cmd_inhibited_cfgs_df,cmd_inhibited_cfgs)
        cfg_based_tc_df = cfg_based_tc_df.filter(
                    ~pl.col("CFG_NO").is_in(self.inhibited_cfgs) 
        ).select(pl.col(["CFG_NO","TC","PARAMETER"]))
    ...
    def create_inhibit_details_excel(self,cmd_inhibited_cfgs_df,cmd_inhibited_cfgs):
        # Create inhibited CFGs report with reasons
        if self.enable_excel_creation or True:
            # Get all unique inhibited CFGs
            all_inhibited_cfgs = list(set(self.inhibited_cfgs))
            
            # Identify source of inhibition
            inhibited_report_data = []
            
            for cfg in all_inhibited_cfgs:
                # Check if inhibited due to INHIBITED_CONFIGS table
                in_direct_inhibited = cfg in self.inhibited_cfgs and cfg not in cmd_inhibited_cfgs
                # Check if inhibited due to commands
                in_cmd_inhibited = cfg in cmd_inhibited_cfgs
                
                if in_cmd_inhibited:
                    # Get the inhibited commands for this CFG
                    inhibited_tc_list = cmd_inhibited_cfgs_df.filter(pl.col("CFG_NO") == cfg)["TC"].unique().to_list()
                    reason = "INHIBITED_COMMAND: " + ", ".join(inhibited_tc_list)
                elif in_direct_inhibited:
                    reason = "INHIBITED_CONFIGS_TABLE"
                else:
                    reason = "UNKNOWN"
                
                inhibited_report_data.append({"INHIBIT_CFGS": cfg, "REASON": reason})
            
            # Create DataFrame and save to Excel
            inhibited_report_df = pl.DataFrame(inhibited_report_data).sort("INHIBIT_CFGS")
            inhibited_report_df.write_excel(generated_excel__path + "inhibited_cfgs_report.xlsx")
