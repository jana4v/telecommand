<template>
  <div class="node-wrap" :style="{ opacity: d.opacity ?? 1 }">
    <svg :width="w" :height="h" :viewBox="`0 0 ${w} ${h}`" xmlns="http://www.w3.org/2000/svg" overflow="visible">
      <defs>
        <linearGradient :id="`qpd-rim-${uid}`" x1="0" y1="0" x2="1" y2="1">
        ...
        <linearGradient :id="`qpd-body-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#0e1828"/>
          <stop offset="100%" stop-color="#060e18"/>
        </linearGradient>
        <radialGradient :id="`qpd-panel-off-${uid}`" cx="50%" cy="45%" r="65%">
          <stop offset="0%" stop-color="#14182e"/>
          <stop offset="100%" stop-color="#06080e"/>
        </radialGradient>
        <radialGradient :id="`qpd-panel-on-${uid}`" cx="40%" cy="35%" r="72%">
          <stop offset="0%" stop-color="#101f2f"/>
          <stop offset="100%" stop-color="#040910"/>
        </radialGradient>
        <linearGradient :id="`qpd-conn-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#5a7a9a"/>
          <stop offset="50%" stop-color="#2a4a6a"/>
          <stop offset="100%" stop-color="#1a2a3a"/>
        </linearGradient>
        <filter :id="`qpd-glow-${uid}`" x="-30%" y="-30%" width="160%" height="160%">
          <feGaussianBlur stdDeviation="2.2" result="blur"/>
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
        <clipPath :id="`qpd-clip-${uid}`">
          <rect :x="px" :y="py" :width="pw" :height="ph" :rx="pr"/>
        </clipPath>
      </defs>

      <rect x="1" y="1" :width="w-2" :height="h-2"
        :rx="cr" :fill="`url(#qpd-body-${uid})`"
        :stroke="`url(#qpd-rim-${uid})`" :stroke-width="rw"
      />

      <rect :x="0" :y="h/2 - connH/2" :width="connW" :height="connH" :rx="connW*0.35"
        :fill="`url(#qpd-conn-${uid})`" stroke="#4a6a8a" :stroke-width="w*0.007"
      />
      ...
      <rect :x="px" :y="py" :width="pw" :height="ph" :rx="pr"
        :fill="isOn ? `url(#qpd-panel-on-${uid})` : `url(#qpd-panel-off-${uid})`"
      ...
      <text :x="titleX" :y="titleY"
        ...
        :fill="titleFontColor"
        :clip-path="`url(#qpd-clip-${uid})`"
      >{{ titleText }}</text>

      <line v-if="!isOn"
        :x1="connW" :y1="sigY" :x2="constCx - constR" :y2="sigY"
      ...
      <path v-else :d="carrierPath"
        ...
        :filter="`url(#qpd-glow-${uid})`"
        :clip-path="`url(#qpd-clip-${uid})`"
      />

      <circle :cx="constCx" :cy="constCy" :r="constR"
        fill="#061526" :stroke="isOn ? '#44ff88' : '#304060'" :stroke-width="1.2"
        :filter="isOn && !d.isInvalid ? `url(#qpd-glow-${uid})` : ''"
        :clip-path="`url(#qpd-clip-${uid})`"
      />
      ...
      <circle v-for="p in constellationPts" :key="p.id" :cx="p.x" :cy="p.y" :r="constR*0.11"
        :fill="isOn ? '#22d3ee' : '#32465a'" opacity="0.85"
        :clip-path="`url(#qpd-clip-${uid})`"
      />
      <circle v-if="isOn" :cx="activePt.x" :cy="activePt.y" :r="constR*0.18"
        fill="#fbbf24" opacity="0.95" :filter="`url(#qpd-glow-${uid})`"
        :clip-path="`url(#qpd-clip-${uid})`"
      />

      <rect :x="demapX" :y="sigY - ph*0.23" :width="demapW" :height="ph*0.46" :rx="ph*0.06"
        :fill="isOn ? '#062214' : '#0b1320'"
        :stroke="isOn ? '#44ff88' : '#1e3048'" stroke-width="1.2"
        :clip-path="`url(#qpd-clip-${uid})`"
      />
      <text :x="demapX + demapW * 0.5" :y="sigY + ph*0.01"
        text-anchor="middle" dominant-baseline="middle"
        :fill="isOn ? '#86efac' : '#4a6070'"
        :font-size="h*0.078" font-family="'Segoe UI', sans-serif" font-weight="700"
        :clip-path="`url(#qpd-clip-${uid})`"
      >DEMAP</text>

      <line v-if="!isOn"
        :x1="demapX + demapW" :y1="sigY" :x2="w - connW" :y2="sigY"
        stroke="#1e3a2a" stroke-width="1.4" stroke-linecap="round" opacity="0.26"
        :clip-path="`url(#qpd-clip-${uid})`"
      />
      <path v-else :d="outputPath"
...
const titleText = computed(() => (d.value.name as string) || "QPSK DEM");
...
const constCx = computed(() => px.value + pw.value * 0.34);
const constCy = computed(() => sigY.value);
const constR = computed(() => ph.value * 0.28);
const demapX = computed(() => px.value + pw.value * 0.53);
const demapW = computed(() => pw.value * 0.19);
...
const INPUT_CARRIER_PER_SYMBOL = 2.2;
const OUTPUT_SPEED = 2.4;
const OUT_VISIBLE = 5;
...
function buildCarrierPath(symbolPhase: number): string {
  ...
  const ex = constCx.value - constR.value;
  const width = ex - sx;
  if (width <= 4) return "";
  ...
  for (let i = 0; i <= samples; i++) {
    ...
    const arg = u * SYMBOLS_VISIBLE * INPUT_CARRIER_PER_SYMBOL * 2 * Math.PI;
    const y = cy - amp * Math.sin(arg + phaseOffset);
...
function buildOutputPath(phase: number): string {
  const sx = demapX.value + demapW.value;
  const ex = w.value - connW.value;
  const width = ex - sx;
  if (width <= 4) return "";

  const seg = width / OUT_VISIBLE;
  const cy = sigY.value;
...
function tick(t: number) {
  ...
  if (isOn.value) {
    phase = (phase + dt * OUTPUT_SPEED) % BITS.length;
    const symbolPhase = phase * 0.5;
    carrierPath.value = buildCarrierPath(symbolPhase);
    outputPath.value = buildOutputPath(phase);

    const symBase = Math.floor(symbolPhase);
