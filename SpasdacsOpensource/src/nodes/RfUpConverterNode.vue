  ...
  <div class="node-wrap" :style="{ opacity: d.opacity ?? 1 }">
    <svg :width="w" :height="h" :viewBox="`0 0 ${w} ${h}`" xmlns="http://www.w3.org/2000/svg" overflow="visible">
      <defs>
        ...
        <linearGradient :id="`ruc-rim-${uid}`" x1="0" y1="0" x2="1" y2="1">
        ...
        <linearGradient :id="`ruc-body-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#0e1828"/>
          <stop offset="100%" stop-color="#060e18"/>
        </linearGradient>
        <linearGradient :id="`ruc-sheen-${uid}`" x1="0" y1="0" x2="0" y2="1">
        ...
        <!-- Inner panel — OFF -->
        <radialGradient :id="`ruc-off-${uid}`" cx="50%" cy="45%" r="65%">
          <stop offset="0%"   stop-color="#14182e"/>
          <stop offset="100%" stop-color="#06080e"/>
        </radialGradient>
        <!-- Inner panel — ON (purple/violet glow) -->
        <radialGradient :id="`ruc-on-${uid}`" cx="38%" cy="38%" r="70%">
          <stop offset="0%"   stop-color="#200a42"/>
          <stop offset="60%"  stop-color="#110528"/>
          <stop offset="100%" stop-color="#060210"/>
        </radialGradient>
        <!-- Mixer circle fill -->
        <radialGradient :id="`ruc-mixer-${uid}`" cx="35%" cy="35%" r="65%">
          <stop offset="0%"   :stop-color="isOn ? '#3b1f6a' : '#18222e'"/>
          <stop offset="100%" :stop-color="isOn ? '#16082e' : '#08101a'"/>
        </radialGradient>
        <!-- RF connector -->
        <linearGradient :id="`ruc-conn-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#5a7a9a"/>
          <stop offset="50%"  stop-color="#2a4a6a"/>
          <stop offset="100%" stop-color="#1a2a3a"/>
        </linearGradient>
        <linearGradient :id="`ruc-conn-rim-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#7a9aba"/>
          <stop offset="50%"  stop-color="#4a6a8a"/>
          <stop offset="100%" stop-color="#1e3048"/>
        </linearGradient>
        <!-- Mixer glow filter -->
        <filter :id="`ruc-glow-${uid}`" x="-35%" y="-35%" width="170%" height="170%">
          <feGaussianBlur stdDeviation="2.2" result="blur"/>
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
        <!-- Clip: inner panel -->
        <clipPath :id="`ruc-clip-${uid}`">
      ...
      <rect x="1" y="1" :width="w-2" :height="h-2"
        :rx="cr" :fill="`url(#ruc-body-${uid})`"
        :stroke="`url(#ruc-rim-${uid})`" :stroke-width="rw"
      />
      <rect :x="rw*0.5" :y="rw*0.5" :width="w-rw" :height="h-rw"
        :rx="cr*0.8" :fill="`url(#ruc-sheen-${uid})`" pointer-events="none"
      />

      <!-- ── RF Connectors ───────────────────────────────────────────────── -->
      <rect :x="0" :y="h/2 - connH/2" :width="connW" :height="connH" :rx="connW*0.35"
        :fill="`url(#ruc-conn-${uid})`" :stroke="`url(#ruc-conn-rim-${uid})`" :stroke-width="w*0.009"
      />
      ...
      <!-- ── Inner panel ─────────────────────────────────────────────────── -->
      <rect :x="px" :y="py" :width="pw" :height="ph" :rx="pr"
        :fill="isOn ? `url(#ruc-on-${uid})` : `url(#ruc-off-${uid})`"
        stroke="#06080e" :stroke-width="rw*0.4"
      />
      <!-- Panel glow when ON -->
      <rect v-if="isOn && !d.isInvalid" :x="px" :y="py" :width="pw" :height="ph" :rx="pr"
        fill="none" stroke="#a855f7" :stroke-width="rw*0.4" opacity="0.35"
        :style="`filter: drop-shadow(0 0 ${pw*0.04}px #a855f7)`"
      />

      <!-- ── Device name ─────────────────────────────────────────────────── -->


      <!-- ── Signal line: IF IN → mixer circle ──────────────────────────── -->
      <line :x1="connW" :y1="mixerCy"
        ...
        :clip-path="`url(#ruc-clip-${uid})`"
      />
      <!-- ── Signal line: mixer circle → RF OUT ─────────────────────────── -->
      ...
      <circle :cx="mixerCx" :cy="mixerCy" :r="mixerR"
        :fill="`url(#ruc-mixer-${uid})`"
        :stroke="mixerStroke" :stroke-width="w*0.013"
        :filter="isOn && !d.isInvalid ? `url(#ruc-glow-${uid})` : ''"
      />
      ...
      <!-- ── Up-conversion chevrons (↑) to the right of mixer ───────────── -->
      <path :d="chevron1"
...
// ── Up-conversion chevron positions ──────────────────────────────────────
// Two upward-pointing ∧ shapes to the right of the mixer circle
...
// ── Colors (purple/violet accent for up-conversion) ───────────────────────
const signalColor  = computed(() => d.value.isInvalid ? "#e74c3c" : isOn.value ? "#a855f7" : "#2a3a4a");
const loColor      = computed(() => d.value.isInvalid ? "#e74c3c" : isOn.value ? "#c084fc" : "#1e2a3a");
const mixerStroke  = computed(() => d.value.isInvalid ? "#e74c3c" : isOn.value ? "#a855f7" : "#2a4060");
const crossColor   = computed(() => d.value.isInvalid ? "#e74c3c" : isOn.value ? "#d8b4fe" : "#3a5070");
