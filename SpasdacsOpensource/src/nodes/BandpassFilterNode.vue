<template>
  <div class="node-wrap" :style="{ opacity: d.opacity ?? 1 }">
    <svg :width="w" :height="h" :viewBox="`0 0 ${w} ${h}`" xmlns="http://www.w3.org/2000/svg" overflow="visible">
      <defs>
        <!-- ── Metallic chassis ────────────────────────────────────── -->
        <linearGradient :id="`bpf-rim-${uid}`" x1="0" y1="0" x2="1" y2="1">
        ...
        <linearGradient :id="`bpf-body-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#0e1828"/>
          <stop offset="100%" stop-color="#060e18"/>
        </linearGradient>
        <linearGradient :id="`bpf-sheen-${uid}`" x1="0" y1="0" x2="0" y2="1">
        ...
        <radialGradient :id="`bpf-panel-off-${uid}`" cx="50%" cy="45%" r="65%">
          <stop offset="0%"   stop-color="#14182e"/>
          <stop offset="100%" stop-color="#06080e"/>
        </radialGradient>
        <radialGradient :id="`bpf-panel-on-${uid}`" cx="38%" cy="38%" r="70%">
          <stop offset="0%"   stop-color="#1a0e38"/>
          <stop offset="60%"  stop-color="#0c0620"/>
          <stop offset="100%" stop-color="#05030f"/>
        </radialGradient>

        <!-- ── Filter curve fill ──────────────────────────────────── -->
        <linearGradient :id="`bpf-fill-on-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#a78bfa" stop-opacity="0.38"/>
          <stop offset="100%" stop-color="#6d28d9" stop-opacity="0.06"/>
        </linearGradient>
        <linearGradient :id="`bpf-fill-off-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#475569" stop-opacity="0.22"/>
          <stop offset="100%" stop-color="#1e293b" stop-opacity="0.05"/>
        </linearGradient>

        <!-- ── RF connector ───────────────────────────────────────── -->
        <linearGradient :id="`bpf-conn-${uid}`" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#5a7a9a"/>
          <stop offset="50%"  stop-color="#2a4a6a"/>
          <stop offset="100%" stop-color="#1a2a3a"/>
        </linearGradient>
        <linearGradient :id="`bpf-conn-rim-${uid}`" x1="0" y1="0" x2="0" y2="1">
        ...
        <!-- ── Glow filter ────────────────────────────────────────── -->
        <filter :id="`bpf-glow-${uid}`" x="-30%" y="-30%" width="160%" height="160%">
          <feGaussianBlur stdDeviation="2.2" result="blur"/>
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>

        <!-- ── Clip: inner panel ──────────────────────────────────── -->
        <clipPath :id="`bpf-clip-${uid}`">
      ...
      <rect x="1" y="1" :width="w-2" :height="h-2"
        :rx="cr" :fill="`url(#bpf-body-${uid})`"
        :stroke="`url(#bpf-rim-${uid})`" :stroke-width="rw"/>
      <rect :x="rw*0.5" :y="rw*0.5" :width="w-rw" :height="h-rw"
        :rx="cr*0.8" :fill="`url(#bpf-sheen-${uid})`" pointer-events="none"/>

      <!-- ── RF connectors (left + right) ───────────────────────────────── -->
      <rect :x="0"     :y="h/2 - connH/2" :width="connW" :height="connH" :rx="connW*0.35"
        :fill="`url(#bpf-conn-${uid})`" :stroke="`url(#bpf-conn-rim-${uid})`" :stroke-width="w*0.009"/>
      ...
      <!-- ── Inner panel ─────────────────────────────────────────────────── -->
      <rect :x="px" :y="py" :width="pw" :height="ph" :rx="pr"
        :fill="showActive ? `url(#bpf-panel-on-${uid})` : `url(#bpf-panel-off-${uid})`"
      ...
      <rect v-if="showActive && !d.isInvalid" :x="px" :y="py" :width="pw" :height="ph" :rx="pr"
        fill="none" stroke="#a78bfa" :stroke-width="rw*0.35" opacity="0.36"
        :style="`filter: drop-shadow(0 0 ${pw*0.04}px #7c3aed)`"/>
      ...
      <line :x1="curveX0" :y1="axisY" :x2="curveX5" :y2="axisY"
        ...
        :clip-path="`url(#bpf-clip-${uid})`"/>

      <!-- ── Signal path lines (IN → curve start, curve end → OUT) ──────── -->
      <line :x1="connW" :y1="h/2" :x2="curveX0" :y2="axisY"
        :stroke="sigColor" stroke-width="1.5" stroke-linecap="round"
        :opacity="showActive ? 0.7 : 0.2" :clip-path="`url(#bpf-clip-${uid})`"/>
      <line :x1="curveX5" :y1="axisY" :x2="px + mainW" :y2="h/2"
        :stroke="sigColor" stroke-width="1.5" stroke-linecap="round"
        :opacity="showActive ? 0.7 : 0.2" :clip-path="`url(#bpf-clip-${uid})`"/>

      <!-- ── Filter curve: filled area ─────────────────────────────────── -->
      <path :d="curveFillPath"
        :fill="showActive ? `url(#bpf-fill-on-${uid})` : `url(#bpf-fill-off-${uid})`"
        :clip-path="`url(#bpf-clip-${uid})`"/>
      ...
      <path :d="curvePath"
        ...
        :filter="showActive && !d.isInvalid ? `url(#bpf-glow-${uid})` : ''"
        :clip-path="`url(#bpf-clip-${uid})`"/>

      <!-- ── Cutoff markers (dashed verticals at band edges) ────────────── -->
      <line :x1="bpX2" :y1="topY - ph*0.04" :x2="bpX2" :y2="axisY"
        :stroke="curveStroke" :stroke-width="curveLineW*0.45"
        stroke-dasharray="2.5,2.5" opacity="0.40"
        :clip-path="`url(#bpf-clip-${uid})`"/>
      <line :x1="bpX3" :y1="topY - ph*0.04" :x2="bpX3" :y2="axisY"
        :stroke="curveStroke" :stroke-width="curveLineW*0.45"
        stroke-dasharray="2.5,2.5" opacity="0.40"
        :clip-path="`url(#bpf-clip-${uid})`"/>

      <!-- ── Centre-frequency tick ──────────────────────────────────────── -->
      <line :x1="fcX" :y1="topY - ph*0.08" :x2="fcX" :y2="topY + ph*0.06"
        :stroke="curveStroke" :stroke-width="curveLineW*0.55"
        opacity="0.60" :clip-path="`url(#bpf-clip-${uid})`"/>

      <!-- ── Separator before right strip (only when LED shown) ───────────── -->
      <line v-if="hasStatusBinding" :x1="px + mainW" :y1="py + ph*0.08" :x2="px + mainW" :y2="py + ph*0.65"
      ...
      <text v-if="d.name"
        ...
        :clip-path="`url(#bpf-clip-${uid})`">{{ d.name }}</text>

      <!-- ── Center frequency label (below curve, when bound) ───────────── -->
      <text v-if="centerFreqText"
        :x="fcX" :y="py + ph*0.90"
        text-anchor="middle" dominant-baseline="middle"
        :fill="curveStroke" :font-size="subFontSize"
        font-weight="600" font-family="'Segoe UI',sans-serif" opacity="0.88"
        :clip-path="`url(#bpf-clip-${uid})`">{{ centerFreqText }}</text>
      ...
      <text v-if="hasTempBinding"
        ...
        :clip-path="`url(#bpf-clip-${uid})`">{{ tempText }}</text>
...
// Y: topY = passband peak   botY/axisY = rejection floor (also the frequency axis)
const topY  = computed(() => py.value + ph.value * 0.24);
const botY  = computed(() => py.value + ph.value * 0.76);
const axisY = computed(() => botY.value);

// X: spans from just after the left connector to the right edge of mainW
const curveX0 = computed(() => px.value + connW.value + pw.value * 0.01);
const curveX5 = computed(() => px.value + mainW.value  - pw.value * 0.01);

// Transition band boundaries (as fractions of curve span)
const bpX1 = computed(() => curveX0.value + (curveX5.value - curveX0.value) * 0.18);
const bpX2 = computed(() => curveX0.value + (curveX5.value - curveX0.value) * 0.31);
const bpX3 = computed(() => curveX0.value + (curveX5.value - curveX0.value) * 0.69);
const bpX4 = computed(() => curveX0.value + (curveX5.value - curveX0.value) * 0.82);
/** Centre frequency x position */
const fcX   = computed(() => (bpX2.value + bpX3.value) / 2);

// ── Filter response path (smooth cubic bezier S-curves on the roll-offs) ──
const curvePath = computed(() => {
  const x0 = curveX0.value, x1 = bpX1.value, x2 = bpX2.value;
  const x3 = bpX3.value,   x4 = bpX4.value, x5 = curveX5.value;
  const tY = topY.value, bY = botY.value;
  const dx = (x2 - x1) * 0.5;
  return (
    `M ${x0},${bY} L ${x1},${bY} ` +
    `C ${x1+dx},${bY} ${x2-dx},${tY} ${x2},${tY} ` +
    `L ${x3},${tY} ` +
    `C ${x3+dx},${tY} ${x4-dx},${bY} ${x4},${bY} ` +
    `L ${x5},${bY}`
  );
});

/** Closed fill shape under the response curve */
const curveFillPath = computed(() => curvePath.value + " Z");
...
/** Violet accent when active; grey when off; red when invalid */
const curveStroke  = computed(() => d.value.isInvalid ? "#e74c3c" : showActive.value ? "#a78bfa" : "#4a5568");
const sigColor     = computed(() => d.value.isInvalid ? "#e74c3c" : showActive.value ? "#c4b5fd" : "#334155");
...
const tempText = computed(() => {
  ...
  return (t as number).toFixed(1) + "°C";
});
