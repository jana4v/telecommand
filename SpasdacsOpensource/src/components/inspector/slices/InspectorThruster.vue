<template>
  <section class="section">
    <h4 class="section-title">Thruster Settings</h4>

    <!-- Label -->
    <label class="field">
      <span>Label</span>
      <input class="input" type="text" placeholder="e.g. THR-A1"
        :value="data.name ?? ''"
    ...
    <!-- Propellant valves -->
    <div class="field">
      <span>Propellant Valves</span>
      <div class="valve-row">
        <!-- Oxidizer -->
        <div class="valve-block">
          <div class="valve-label ox-label">OXIDIZER</div>
          <button class="valve-btn" :class="{ 'valve-on': oxOn, 'ox-btn': true }"
            @click="set('oxidizerEnabled', !oxOn)">
            <span class="valve-dot" :class="{ 'dot-ox-on': oxOn }"/>
            {{ oxOn ? 'OPEN' : 'CLOSED' }}
          </button>
        </div>
        <!-- Fuel -->
        <div class="valve-block">
          <div class="valve-label fu-label">FUEL</div>
          <button class="valve-btn" :class="{ 'valve-on': fuOn, 'fu-btn': true }"
            @click="set('fuelEnabled', !fuOn)">
            <span class="valve-dot" :class="{ 'dot-fu-on': fuOn }"/>
            {{ fuOn ? 'OPEN' : 'CLOSED' }}
          </button>
        </div>
      </div>
      <span class="field-hint">Both valves must be open for bipropellant firing</span>
    </div>

    <div class="divider"/>

    <!-- Fire / Shutdown test controls -->
    <div class="field">
      <span>Quick Test</span>
      <div class="btn-group">
        <button class="btn btn-fire"
          @click="() => { set('thrustLevel', 1); set('oxidizerEnabled', true); set('fuelEnabled', true); }">
          🔥 Fire Max
        </button>
        <button class="btn btn-shutdown"
          @click="() => { set('thrustLevel', 0); set('oxidizerEnabled', false); set('fuelEnabled', false); }">
          ⬛ Shutdown
        </button>
      </div>
    </div>
    ...
    <div class="state-panel">
      ...
      <div class="state-row">
        <span class="state-key">OX</span>
        <span class="state-chip" :class="oxOn ? 'chip-ox' : 'chip-off'">
          {{ oxOn ? '● OPEN' : '○ CLOSED' }}
        </span>
      </div>
      <div class="state-row">
        <span class="state-key">FUL</span>
      ...
      <div class="thrust-bar-wrap">
        <div class="thrust-bar-track">
          <div class="thrust-bar-fill" :style="{ width: thrustPct + '%' }"
            :class="thrustClass"/>
        </div>
...
const oxOn = computed(() => {
  const v = props.data.oxidizerEnabled;
...
.slider {
  flex: 1; accent-color: #4a9eff; cursor: pointer;
}
...
.btn-fire     { background: #1a0a04; color: #ff6820; border-color: #501808; }
.btn-fire:hover { background: #2a1008; color: #ff8040; }
.btn-shutdown { background: #0d1117; color: #6e7681; border-color: #30363d; }

/* ── Valve controls ── */
.valve-row   { display: flex; gap: 6px; }
.valve-block { flex: 1; display: flex; flex-direction: column; gap: 3px; }
.valve-label { font-size: 9px; font-weight: 700; letter-spacing: 0.08em; text-align: center; }
.ox-label    { color: #3068c0; }
.fu-label    { color: #b84020; }
.valve-btn {
...
.valve-on.ox-btn { background: #0a1630; color: #4888ff; border-color: #1840a0; }
.valve-on.fu-btn { background: #200a04; color: #ff6030; border-color: #901808; }
.valve-dot {
  width: 6px; height: 6px; border-radius: 50%;
  background: #2a3040; display: inline-block; flex-shrink: 0;
}
.dot-ox-on { background: #2880ff; box-shadow: 0 0 4px #2880ff; }
.dot-fu-on { background: #ff5020; box-shadow: 0 0 4px #ff5020; }
...
.state-key    { font-size: 9px; font-weight: 700; font-family: monospace; color: #4a5568; width: 28px; }
.state-chip {
...
.chip-ox   { background: #08101e; color: #3880ff; border-color: #183880; }
...
.thrust-bar-track {
  flex: 1; height: 5px; background: #0c1018;
  border: 1px solid #1c2030; border-radius: 3px; overflow: hidden;
}
.thrust-bar-fill {
  height: 100%; border-radius: 3px; transition: width 0.15s ease;
}
